...
function gameend(socket,data){
	var username=data.username;
	var score=data.score;
	var userID=data.userID;
	io.sockets.emit('finish', { username: username, score: score, userID: userID});
}
	...
	db.collection('users', function (err,collection) {
		collection.find().toArray(function(err,users){
			...
			if(!flaga){
				...
				io.sockets.emit('loged', { username: username, userID: userID, realID: ID, createdID: createdID});
			}
		});
	});
